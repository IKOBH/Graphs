...
class Graph(object):
    ...
    def get_edge_by_objs(self, exit_obj, enter_obj):
        ...
        If exists return edge s.t:
        * edge.exit_node.data = exit.obj.
        * edge.enter_node.data = enter.obj.
        Else return None.
    ...
    def add_directed_edge(self, exit_obj, enter_obj):
        ...
        If not exists, add edge s.t:
        * exit_obj as it's exit node's data.
        * enter_obj as it's enter node's data.
    ...
    def del_edges_from(self, objs):
        ...
        Delete edges between exit node with data exit_obj & enter node with data enter_obj,
        for each (exitObj, enterObj) tuple.
...
class Edge(object):
    ...
    def __init__(self, exit_node, enter_node):
        ...
        if not isinstance(exit_node, Node) or not isinstance(enter_node, Node):
            raise TypeError("Node objects expected, got (%s,%s)" %
                            (exit_node.__class__.__name__, enter_node.__class__.__name__))
...
class Node(object):
    ...
    def __init__(self, obj=None):
        ...
        '''
        self.id = id(self)
        self.data = self.id if obj is None else obj
        self.exit_edges = set()
